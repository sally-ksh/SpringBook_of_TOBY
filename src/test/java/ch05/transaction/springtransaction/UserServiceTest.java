...
class UserServiceTest {
	...
	void upgradeAllOrNothing() {
		ch05.transaction.UserService testUserService = new ch05.transaction.UserService(this.userDao, new UserServiceTest.TestUserLevelUpgrade(users.get(3).getId()), dataSource);

		userDao.deleteAll();
		users.values().forEach(userDao::add);

		assertThrows(UserServiceTest.TestUserServiceException.class, () -> testUserService.upgradeLevels());
	...
	static class TestUserLevelUpgrade implements UserLevelUpgradePolicy {
		...
		public boolean canUpgradeLevel(User user) {
			...
			switch (currentLevel) {
				case BASIC:
					return (user.getLogin() >= ch05.transaction.UserService.MIN_LOG_COUNT_FOR_SILVER);
				case SILVER:
